...
- HealthKit returns no sleep data in the Simulator. Full functionality (reading sleep sessions)
  requires a **physical device** with sleep data in the Health app, as noted in `README.md`.
...
- Code changes can still be made and reviewed on Linux, but verification (build/run/HealthKit
  behavior) must be done by a human (or macOS CI) on macOS + Xcode, ideally on a physical device.
